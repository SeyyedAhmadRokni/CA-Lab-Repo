...
module EXE_Stage(clk, rst, WB_ENIn, MEM_R_ENIn, MEM_W_ENIn, EXE_CMDIn, 
    ...
    input wire clk, rst, WB_ENIn, MEM_R_ENIn, MEM_W_ENIn, SIn, IIn;
    ...
    output wire WB_ENOut, MEM_R_ENOut, MEM_W_ENOut, SOut;
    output wire[3:0] DestOut, statusOut;
    output wire[31:0] ALU_ResOut, Val_RmOut, branchAddressOut;


    wire STypeSignal = MEM_R_ENIn | MEM_W_ENIn;
    ...
    adder branchAddressAdder(
        .a(PCIn), .b({{6{Imm24In[23]}}, Imm24In, 2'b00}), .sum(branchAddressOut),
        .carry(carry)
    );
